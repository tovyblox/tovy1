  ...
  <div>
    ...
    <v-container class="mt-n16 mx-auto">
      ...
      <v-card outlined>
        ...
        <div class="mx-4">
          <v-form>
            <v-text-field hide-details="auto" v-model="username" outlined label="Username" />
            <v-textarea class="mt-2"  hide-details="auto" v-model="reason" label="Reason" outlined />
            <v-btn class="mt-2" color="primary" v-bind:disabled="perm == true" @click.stop="dialog.active = true" block>Select end date</v-btn>
        ...
        <v-card>
              ...
              v-model="date"
            ></v-date-picker>
          <v-btn class=" mb-8" color="primary" @click.stop="dialog.active = false" block>Submit</v-btn>
          </v-card>
          </v-dialog>
      ...
      <v-card outlined class="mt-8">
        <v-card-text> Lookup </v-card-text>
        <div class="mx-4">
          <v-form>
            <v-text-field hide-details="auto" v-model="username" label="Username" outlined />
          </v-form>
          <v-row class="mb-4 mx-0">
            <v-btn @click="look()" text color="success" class="ml-auto mt-7">
              Lookup
            </v-btn>
          </v-row>
        </div>
...
export default {
  ...
  methods: {
    search() {
      this.loading = true;
      this.user.name = this.dataobject;
      this.$http.get("/pfp/name/" + this.dataobject).then((response) => {
        this.user.picture = response.data.pfp;
      });
      this.loading = false;
    },
    goto: function (url) {
